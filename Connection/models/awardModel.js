...
function findTopActors() {
    return new Promise((resolve, reject) => {
        let sql = "SELECT NAME, COUNT(*) As total, sum(case when Won = 'True\\r' then 1 else 0 end) As WonCount FROM " +
            "ScreenActorGuildAwards GROUP BY NAME HAVING NAME <> '' ORDER BY sum(case when Won = 'True\\r' then " +
...
function findYearsOfAwardsByActor(name) {
    return new Promise((resolve, reject) => {
        let sql =
            "SELECT NAME, CAST(LEFT(YEAR, 4) AS SIGNED) AS Year, COUNT(*) AS NumberOfAwards FROM ScreenActorGuildAwards WHERE Won = 'True' GROUP BY NAME, YEAR HAVING NAME = ?";
        const inserts = [name];
...
function findAllStats() {
    return new Promise((resolve, reject) => {
        connection.query("SELECT SUBSTRING_INDEX(YEAR, ' ', 1) AS YEAR, COUNT(*) AS TOTAL, SUM(CASE WHEN WON='True\\r' THEN 1 ELSE 0 END) AS WINS, SUM(CASE WHEN WON='False\\r' THEN 1 ELSE 0 END) AS LOSSES FROM screenactorguildawards GROUP BY YEAR HAVING YEAR LIKE '%-%'", function (err, result, fields) {
            if (err)
                throw err
            resolve(result)
        })
...
function findRandomActors() {
    return new Promise((resolve, reject) => {
        connection.query("select name from screenactorguildawards where name <> '' and (year like '2020%' or year like '2019%' or year like '2018%') group by name having sum(case when won='True\\r' then 1 else 0 end) > 0 order by rand() limit 0, 10;", function (err, result, fields) {
            if (err)
                throw err
            resolve(result)
        })
